...
## 5. Aggregazione gli effect size within-study
...
```{r}
# Informazioni a livello di studio: una sola riga per studio
study_info_author <- aggregate(
  author ~ study_id,
  data = dat,
  FUN = function(x) x[1]
)

study_info_year <- aggregate(
  pub_year ~ study_id,
  data = dat,
  FUN = function(x) x[1]
)
...
# Rimuoviamo eventuali colonne problematiche già presenti in dat_agg
dat_agg$author <- NULL
dat_agg$pub_year <- NULL

# Aggiungiamo autore e anno puliti al dataset aggregato
dat_agg <- merge(
  dat_agg,
  study_info,
  by = "study_id",
...
# Creiamo etichette per il forest plot
dat_agg$slab <- paste0(dat_agg$author, " (", dat_agg$pub_year, ")")

# Ordiniamo per anno di pubblicazione
...
- L'intervallo di fiducia descrive l'incertezza sulla stima media.
